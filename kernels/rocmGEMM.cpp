...
void run_batch_gemm(int version) {
    ...
    HIP_CHECK(hipMalloc(&d_A, mat_bytes));
    HIP_CHECK(hipMalloc(&d_B, mat_bytes));
    
    // Allocate a contiguous 4GB block for 1000 output matrices
    HIP_CHECK(hipMalloc(&d_C_batch, NUM_RUNS * mat_bytes));
    
    HIP_CHECK(hipMemcpy(d_A, h_A, mat_bytes, hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpy(d_B, h_B, mat_bytes, hipMemcpyHostToDevice));
    ...
    hipLaunchKernelGGL(gemm_v0, grid, block, 0, 0, d_A, d_B, d_C_batch);
    ...
    if (version < 4) {
        ...
        for(int i = 0; i < NUM_RUNS; i++) {
            ...
            if (version == 0) hipLaunchKernelGGL(gemm_v0, grid, block, 0, 0, d_A, d_B, d_C_current);
            else if (version == 1) hipLaunchKernelGGL(gemm_v1, grid, block, 0, 0, d_A, d_B, d_C_current);
            else if (version == 2) hipLaunchKernelGGL(gemm_v2_unrolled, grid, block, 0, 0, d_A, d_B, d_C_current);
            else if (version == 3) hipLaunchKernelGGL(gemm_v3_vectorized, grid, block, 0, 0, d_A, d_B, d_C_current);
        }
    } 
    else if (version == 4) {
        ...
        for(int i = 0; i < NUM_RUNS; i++) {
            ...
            float* d_C_current = d_C_batch + (i * mat_elements);
            hipLaunchKernelGGL(gemm_v3_vectorized, grid, block, 0, streams[stream_idx], d_A, d_B, d_C_current);
        }
    ...
    // 5. Cleanup
    hipFree(d_A); hipFree(d_B); hipFree(d_C_batch);
