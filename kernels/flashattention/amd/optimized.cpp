 ...
 *   Wavefront 0:         S = Q·Kᵀ (16×16 rocWMMA output)
 *   Threads 0-(Br-1):    scalar online softmax (one thread per Q-row)
 *   All wavefronts:      O += P·V, striding over D in chunks of WMMA_N
 *
 * Requires: ROCm with rocWMMA; CDNA (gfx9xx) or RDNA3 (gfx11xx).
 * For RDNA3 set WAVEFRONT_SIZE=32 in config.h.
 *
 * Compile:
 *   hipcc -O3 --offload-arch=gfx90a optimized.cpp -o optimized_amd
 */
...
// Shared memory per block (~43 KB at D=256, fits in 64 KB LDS on CDNA):
...
void flash_attention_optimized(
    ...
    for (int kv_tile = 0; kv_tile < num_kv_tiles; kv_tile++) {
        ...
        // S = Q · Kᵀ via rocWMMA (wavefront 0 only).
        ...
        if (wavefront_id == 0) {
            ...
            for (int k = 0; k < D; k += WMMA_K) {
                load_matrix_sync(q_frag, &Q_smem[0][k], D + PAD);
                load_matrix_sync(k_frag, &K_smem[0][k], D + PAD);
                mma_sync(s_frag, q_frag, k_frag, s_frag);
            }
            store_matrix_sync(&S_smem[0][0], s_frag, Bc, mem_row_major);
        }
        __syncthreads();
        ...
        // O += P · V via rocWMMA.
        // Wavefronts stride over D in steps of NUM_WAVEFRONTS*WMMA_N so the
        // kernel handles any D that is a multiple of WMMA_N.
        {
            ...
            load_matrix_sync(p_frag, &P_smem[0][0], Bc + PAD);

            for (int col_offset = wavefront_id * WMMA_N;
                     col_offset < D;
                     col_offset += NUM_WAVEFRONTS * WMMA_N) {
                // Seed accumulator with rescaled O so mma_sync adds P·V to it.
                load_matrix_sync(o_frag, &O_smem[0][col_offset], D, mem_row_major);
                load_matrix_sync(v_frag, &V_smem[0][col_offset], D + PAD);
                mma_sync(o_frag, p_frag, v_frag, o_frag);
                store_matrix_sync(&O_smem[0][col_offset], o_frag, D, mem_row_major);
            }
        }
        __syncthreads();
...
int main() {
    ...
    size_t mat_size_f16 = N * d * sizeof(__half);
