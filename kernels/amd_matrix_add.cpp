...
__global__ void addMatrices(const float (*A)[N], const float (*B)[N], float (*C)[N]) {

	int globalX = threadIdx.x + blockDim.x * blockIdx.x;
	int globalY = threadIdx.y + blockDim.y * blockIdx.y;
	
	if (globalX < N && globalY < N)
		C[globalY][globalX] = A[globalY][globalX] + B[globalY][globalX];
...
int main() {
	...
	int numBlocks = (N*N + numThreadsPerBlock - 1) / numThreadsPerBlock;
	
	hipEventRecord(start);
	addMatrices<<<numBlocks, numThreadsPerBlock>>>(d_A, d_B, d_C);
