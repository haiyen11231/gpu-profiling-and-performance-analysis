...
float run_batch_gemm(int version, float lastMilliseconds, float naiveMilliseconds) {
    ...
    HIP_CHECK(hipHostMalloc(&h_C_batch, NUM_RUNS * mat_bytes));
    init_matrix(h_A, mat_elements, 42); init_matrix(h_B, mat_elements, 42);
    
    float *d_A, *d_B, *d_C_batch;
    HIP_CHECK(hipMalloc(&d_A, mat_bytes)); HIP_CHECK(hipMalloc(&d_B, mat_bytes));
    HIP_CHECK(hipMalloc(&d_C_batch, NUM_RUNS * mat_bytes));
    HIP_CHECK(hipMemset(d_C_batch, 0, NUM_RUNS * mat_bytes));
    ...
    if (version < 4) {
		...
		if (sizeof(VEC_TYPE)/sizeof(float) == 1) milliseconds = lastMilliseconds;
            ...
            HIP_CHECK(hipMemcpy(h_C_current, d_C_current, mat_bytes, hipMemcpyDeviceToHost));
        }
    } 
    else if (version == 4) {
	...
	if (NUM_STREAMS == 1) {
		milliseconds = lastMilliseconds;
        	...
        	for(int i = 0; i < NUM_RUNS; i++) {
            		...
            		HIP_CHECK(hipMemcpyAsync(h_C_current, d_C_current, mat_bytes, hipMemcpyDeviceToHost, streams[stream_idx]));
        	}
    ...
    hipFree(d_A); hipFree(d_B); hipFree(d_C_batch); hipHostFree(h_C_batch);
    delete[] h_A; delete[] h_B; hipEventDestroy(start); hipEventDestroy(stop);
